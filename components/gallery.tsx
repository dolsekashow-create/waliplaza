          ...
          return (
            <li key={image} className="relative">
              <button
                type="button"
                onClick={() => setOpen(i)}
                aria-label={`تكبير ${category} رقم ${i + 1}`}
                className={cn(
              ...
              <button
                ...
                aria-label={
                  selected
                    ? `إزالة ${category} رقم ${i + 1} من الطلب`
                    : `إضافة ${category} رقم ${i + 1} للطلب`
                }
                className={cn(
          ...
          >
            ...
            >
              ...
              <p className="text-sm font-bold text-white/80">
                {category} — رقم {open + 1} من {images.length}
              </p>

              <div className="flex w-full max-w-md flex-wrap items-center justify-center gap-2">
                <a
                  href={whatsappLink(buildSingleMessage(item(open)))}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-[#25D366] px-5 text-sm font-extrabold text-white transition-colors hover:bg-[#1da851]"
                >
